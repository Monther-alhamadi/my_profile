// Run once: supabase functions serve seed --env-file .env
// or: curl -X POST https://<project>.supabase.co/functions/v1/seed -H "Authorization: Bearer <anon>" -H "x-admin-key: <service_role>"
...
async function insertAll() {
  // --- PROFILE ---
  const { error: pErr } = await supabaseAdmin.from("profile").upsert({
  ...
  const projects = [
    {
      id: "cachear-pos", locale: "en", sort_order: 1, number: "01",
    ...
    {
      id: "cachear-pos", locale: "ar", sort_order: 1, number: "01",
    ...
    {
      id: "heic-converter", locale: "en", sort_order: 2, number: "02",
    ...
    {
      id: "heic-converter", locale: "ar", sort_order: 2, number: "02",
    ...
    {
      id: "university-scheduler", locale: "en", sort_order: 3, number: "03",
    ...
    {
      id: "university-scheduler", locale: "ar", sort_order: 3, number: "03",
    ...
    {
      id: "ai-image-platform", locale: "en", sort_order: 4, number: "03",
    ...
    {
      id: "ai-image-platform", locale: "ar", sort_order: 4, number: "03",
    ...
    {
      id: "nextvendors-ecommerce", locale: "en", sort_order: 5, number: "04",
    ...
    {
      id: "nextvendors-ecommerce", locale: "ar", sort_order: 5, number: "04",
    ...
    {
      id: "ai-tools-hub", locale: "en", sort_order: 6, number: "05",
    ...
    {
      id: "ai-tools-hub", locale: "ar", sort_order: 6, number: "05",
  ...
  const { error: prErr } = await supabaseAdmin.from("projects").upsert(projects, { onConflict: "id, locale" });
  ...
  const { error: skErr } = await supabaseAdmin.from("skills").insert(skills);
  if (skErr) return { error: skErr };
  ...
  const { error: svErr } = await supabaseAdmin.from("services").upsert(services, { onConflict: "id, locale" });
  if (svErr) return { error: svErr };
  ...
  const { error: exErr } = await supabaseAdmin.from("experience").upsert(experience, { onConflict: "id, locale" });
  ...
  const { error: stErr } = await supabaseAdmin.from("stats").upsert(stats, { onConflict: "id, locale" });
  ...
  const { error: teErr } = await supabaseAdmin.from("testimonials").upsert(testimonials, { onConflict: "id, locale" });
  if (teErr) return { error: teErr };
...
Deno.serve(async (req) => {
  ...
  if (adminKey !== serviceKey && !authHeader.includes("service_role")) {
  ...
  try {
    ...
    return jsonResponse({ message: "Seed completed successfully", tables: ["profile", "projects", "skills", "services", "experience", "stats", "testimonials"] });
  } catch (err) {
